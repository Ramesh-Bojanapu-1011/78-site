...
const BmiCalculator = () => {
  ...
  const getBmiStatus = (bmi) => {
    if (bmi < 18.5)
      return { status: t("bmi.results.underweight"), color: "text-blue-600" };
    if (bmi >= 18.5 && bmi <= 24.9)
      return { status: t("bmi.results.healthy"), color: "text-green-600" };
    if (bmi >= 25 && bmi <= 29.9)
      return { status: t("bmi.results.overweight"), color: "text-yellow-600" };
                  ...
                  {bmiChart.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`text-sm ${
                            row.status === t("bmi.results.underweight")
                              ? "text-blue-600"
                              : row.status === t("bmi.results.healthy")
                                ? "text-green-600"
                                : row.status === t("bmi.results.overweight")
                                  ? "text-yellow-600"
                                  : "text-red-600"
                          }`}
                        >
